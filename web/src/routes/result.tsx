      ...
      {store.ifShowResult ? (
        <div>
          <div class="w-100 flex justify-center">
            <div
              class="mt-10 text-lg font-bold link link-primary"
              onClick$={() => {
          ...
          <div class={`action flex justify-center mt-4`}>
            <ul class="menu menu-horizontal bg-base-100 rounded-box">
              ...
              <li>
                <a
                  class=" tooltip"
                  data-tip={t("short.copyTip@@复制")}
                  onClick$={() => {
                    CopyToClipboard(store.shortUrl).then(() => {
                      showTip(
                        alterState,
                        "success",
                        ifCn ? "复制成功" : "Copy short link Successfully ~",
                        2000
                      );
                    });
                  }}
              ...
              <li>
                <a
                  class=" tooltip"
                  data-tip={t("short.qrcodeTip@@二维码")}
                  onClick$={() => {
                    CopyToClipboard(store.shortUrl).then(() => {
                      GenQrcode();
                    });
